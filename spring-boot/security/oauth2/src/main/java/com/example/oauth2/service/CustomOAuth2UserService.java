...
public class CustomOAuth2UserService extends DefaultOAuth2UserService {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        // access token 으로 제공자 user-info를 호출해 원시 속성 앱을 받아온다.
        OAuth2User  oAuth2User = super.loadUser(userRequest);

        //어떤 제공자로 로그인 했는지 : application.yaml의 registration zl
        String registrationId = userRequest.getClientRegistration().getRegistrationId();

        // 제공자 응답에서 "사용자 식별자" 가 들어있는 속성 키 이름
        // CustomOAuth2User.getName()이 이 키로 식별자를 꺼낼때 사용된다.
        String nameAttributeKey = userRequest.getClientRegistration()
                .getProviderDetails()
                .getUserInfoEndpoint()
                .getUserNameAttributeName();

        // 문자열 registrationId -> 우리 enum()으로 원시 속성 맵 -> 제공자별 파서로 감싼다.
        ...
        // 이 예외 타입이어야 필터가 "인증 실패"로 인식해 ㅊ 보내준다.
