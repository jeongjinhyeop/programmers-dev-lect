...
// POST /loging 요청을 UsernamePasswordAuthenticationFilter가 낚아채서 폼 로그인의 진입점이다.
...
// 그리고 서버는 세션 ID 를 JSESSIONID 라는 쿠키로 브라우저에 내려준다.
// 성공 후 처리는 AuthenticationSuccessHandler가 담당 (기본적으로 원래 가려던 페이지 또는 지정된 페이지로 리다이렉트)
// 실패 시에는 AuthenticationFailureHandler 가 담당 (보통 /login?error로 되돌림)

// 6. 이후 요청 -> 쿠키로 인증 유지
// 한 번 로그인 하면 그 다음 요청부터는 아이디/비밀번호를 다시 보내지 않는다.
// 브라우저가 자동으로 JSESSIONID 쿠키를 실어 보내고, 서버는 이 쿠키로 세션을 찾아 저장해둔 SecurityContext를 복원한다. (이 복원은 SecurityContextHolderFilter 가 담당)
