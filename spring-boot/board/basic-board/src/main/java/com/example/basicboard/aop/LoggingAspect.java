package com.example.basicboard.aop;
...
// * @Aspect
// - " 이 클래스는 공통 기능 (횡단 관심사) 를 모아둔 Aspect다" 라고 선언하는 어노테이션이다.
...
public class LoggingAspect {
    ...
    @Pointcut("execution(* com.example.basicboard.controller..*(..))")
    public void controllerLog() {
        // 메서드 본문(body)는 비워둔다. 실제로 로직이 아니라, "대상을 가르키는 이름표" 역할만 하기 때문
    }
    ...
    public Object logRequest(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        try {
            // 이 코드를 기준으로 요청받은 메서드 실행 전과 실행 후로 나뉜다.
            ...
            long end = System.currentTimeMillis() - start;
            System.out.println("[요청 실패] " + method + " : " + end + "ms" + " : 예외메세지 " + e.getMessage());

            // 잡은 예외를 다시 던진다.
            // - 여기서 예외를 삼켜버리면 컨트롤러는 정상 처리된 것 처럼 보여 버그가 된다.
            throw e;
        }
    }
}
