 ...
 * 3.별도 클래스 전략이 user를 생성자로 받아야 했던 이유를 안다
    ...
    부모 클래스에서(x) > 바깥 메서드의 파라메터(user)를 받아다가 사용(변수 캡처)하기 때문에 별도로 필요한 클래스 파일이 필요없어지게 된다.
    즉 삭제할 클래스파일 내의 user 객체를 별도로 생성하여 파라메터로 받을 필요가 없어지게 된 것 (x)
    > 즉 전략 클래스 내부 유저를 보관할 변수와 생성자가 필요 없다.
