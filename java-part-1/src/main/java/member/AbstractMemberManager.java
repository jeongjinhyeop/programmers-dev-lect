...
public class AbstractMemberManager {
    int totalCnt = 0;
    int memberCnt = 0;
    List<AbstractMember> members = new ArrayList<>();
    private final Scanner sc;
    private final String DIR;
    private final Pattern MEMBER_PATTERN = Pattern.compile("^이름 : (.+?) 이메일 : (.+?) 핸드폰 : (.+?)");
    private final String PHONE_REGEX = "^01[016789]\\d{8}$"; // 01X로 시작하는 숫자 11자리
    private final String EMAIL_REGEX = "^[a-zA-Z0-9._%+-]+@[a-zA-Z0-9.-]+$"; // xxxx@xxxxx 형식

    public AbstractMemberManager(Scanner sc, String DIR) {
        this.sc = sc;
        this.DIR = DIR;
        this.members = loadMembersFromFile(DIR);
        this.memberCnt = this.members.size();
    }
    ...
    public void addMember(List<AbstractMember> members) {
    ...
    public boolean checkEmail(List<AbstractMember> members, String email) {
    ...
    public void selectEmail(List<AbstractMember> members, String email){
        for (int i = 0; i < members.size(); i++) {
    ...
    public void selectName(List<AbstractMember> members, String name){
        boolean flag = false;
        for (int i = 0; i < members.size(); i++) {
    ...
    public void selectAll(List<AbstractMember> members){
    ...
    public void updateMember(List<AbstractMember> members, String email){
        String today = LocalDate.now().toString();
        File file = new File(DIR, today+ ".txt");
        StringBuilder sb = new StringBuilder();
        int idx = -1;
    ...
    public void deleteMember(List<AbstractMember> members, String email){
        String today = LocalDate.now().toString();
        File file = new File(DIR, today+ ".txt");
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < members.size(); i++) {
    ...
    public List<AbstractMember> loadMembersFromFile(String DIR) {
        List<AbstractMember> members = new ArrayList<>();
        String today = LocalDate.now().toString();
        File file = new File(DIR, today+ ".txt");
